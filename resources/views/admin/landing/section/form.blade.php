@extends('admin.layouts.admin',['title'=>'Section Ar pages'])


@section('content')
    <?php  $routeName='admin.section' ?><br>
    <div class="card">
        <div class="card-body">
            <form action="{{ isset($model) ? route($routeName.'.update',$model) :  route($routeName.'.store')}}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="form-group col-6">
                        ...
                        <input type="text" placeholder="url" name="video_url"
                               value="{{old("video_url",isset($model) ? ($model->video_url ?? '') : '')}}"
                               class="form-control">
                        @error("video_url")
                        <span class="text-danger">{{$message}}</span>
                        @enderror
                            ...
                            @foreach(config('app.languages') as $index=>$lang)
                                <div class="tab-pane fade {{$loop->first ? ' active show' : '' }}" id="title-{{$lang}}"
                                   ...
                                   <div class="row">
                                       ...
                                       <div class="form-group col-6">
                                           <label> Button</label>
                                           <input type="text" placeholder="btn {{$lang}}" name="{{$lang}}[btn]"
                                                  value="{{old("$lang.btn",isset($model) ? ($model->translateOrDefault($lang)->btn ?? '') : '')}}"
                                                  class="form-control">
                                           @error("$lang.btn")
                                           <span class="text-danger">{{$message}}</span>
                                           @enderror
