@extends('admin.layouts.admin',['title'=>'Section video'])


@section('content')
    <?php  $routeName='admin.video' ?><br>
    <div class="card">
        <div class="card-body">
            <form action="{{ isset($model) ? route($routeName.'.update',$model) :  route($routeName.'.store')}}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="form-group col-6">
                        ...
                        <input type="text" placeholder="url" name="video_id"
                               value="{{old("video_id",isset($model) ? ($model->video_id ?? '') : '')}}"
                               class="form-control">
                        @error("video_id")
                        <span class="text-danger">{{$message}}</span>
                        @enderror
