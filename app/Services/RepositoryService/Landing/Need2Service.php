...
use App\Repositories\Landing\Need2Repository;
...
class Need2Service
{
    public function __construct(protected Need2Repository $repository,
    ...
    public function store($request)
    {
        ...
        $data['image']=$this->fileUploadService
            ->uploadFile($request->image,'need2s_images');


        $model= $this->repository->save($data,new Need2());
    ...
    public function update($request,$model)
    {
        ...
        if($request->has('image')){
            $data['image']=$this->fileUploadService
                ->replaceFile($request->image,$model->image,'need2s_images');
        }
    ...
    {
        return Cache::rememberForever('need2s',
            function (){
