...
use App\Repositories\Landing\FocusRepository;
use App\Services\FileUploadService;
use Illuminate\Support\Facades\Cache;

class FocusService
{
    public function __construct(protected FocusRepository $repository,
    ...
    public function store($request)
    {
        ...
        $data['image']=$this->fileUploadService
            ->uploadFile($request->image,'focus_images');


        $model= $this->repository->save($data,new Focus());
    ...
    public function update($request,$model)
    {
        ...
        if($request->has('image')){
            $data['image']=$this->fileUploadService
                ->replaceFile($request->image,$model->image,'focus_images');
        }
    ...
    {
        return Cache::rememberForever('focus',
            function (){
