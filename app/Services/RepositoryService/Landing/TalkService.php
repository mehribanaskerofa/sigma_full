...
use App\Repositories\Landing\TalkRepository;
use App\Services\FileUploadService;
use Illuminate\Support\Facades\Cache;

class TalkService
{
    public function __construct(protected TalkRepository $repository,
    ...
    public function store($request)
    {
        ...
        $data['image']=$this->fileUploadService
            ->uploadFile($request->image,'talk_images');


        $model= $this->repository->save($data,new Talk());
    ...
    public function update($request,$model)
    {
        ...
        if($request->has('image')){
            $data['image']=$this->fileUploadService
                ->replaceFile($request->image,$model->image,'talk_images');
        }
    ...
    {
        return Cache::rememberForever('talk',
            function (){
