...
use App\Repositories\Landing\AlbumRepository;
use App\Services\FileUploadService;
use Illuminate\Support\Facades\Cache;

class AlbumService
{
    public function __construct(protected AlbumRepository $repository,
    ...
    public function store($request)
    {
        ...
        $data['image']=$this->fileUploadService
            ->uploadFile($request->image,'album_images');


        $model= $this->repository->save($data,new Album());
    ...
    public function update($request,$model)
    {
        ...
        if($request->has('image')){
            $data['image']=$this->fileUploadService
                ->replaceFile($request->image,$model->image,'album_images');
        }
    ...
    {
        return Cache::rememberForever('album',
            function (){
